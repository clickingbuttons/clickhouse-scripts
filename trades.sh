...
clickhouse-client --query="create function updateIntraday as (price, size, conditions, correction) -> correction = 0 and price != 0 and size != 0 and not(hasAny(conditions, [10, 15, 16, 17, 18, 19, 21, 22, 23, 24, 29, 30, 33, 38, 40, 46, 52, 53]));"
clickhouse-client --query="drop function if exists updateDay"
clickhouse-client --query="create function updateDay as (price, size, conditions, correction) -> not hasAny(conditions, [12]) and updateIntraday(price, size, conditions, correction)"
...
#'FINRA NYSE TRF' = 4,
#'FINRA Nasdaq TRF Carteret' = 4,
#'FINRA Nasdaq TRF Chicago' = 4,
participants="Enum8(
	'NYSE American, LLC' = 1,
	'Nasdaq OMX BX, Inc.' = 2,
	'NYSE National, Inc.' = 3,
